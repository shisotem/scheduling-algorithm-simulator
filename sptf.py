"""
Description: This program simulates the Shortest Processing Time First scheduling algorithm.
...
def execute_task(current_task: Task, current_time: int, queue: List[Task], task_list: List[Task]) -> int:
    end_time = current_time + current_task.cost - current_task.progress
...
def run_shortest_processing_time_first(task_list: List[Task]) -> None:
    queue: List[Task] = []
    current_time = 0
    ...
    while finished_task_count != num_tasks:
        ...
        current_time = execute_task(current_task, current_time, queue, task_list)
...
def main() -> None:
    ...
    run_shortest_processing_time_first(task_list)
